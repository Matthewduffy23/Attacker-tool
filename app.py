...
# ==== TOP HEADER (toggleable) ====
import math

# Palette (same as your board)
PAGE_BG = "#0a0f1c"
LABEL   = "#e8eef8"
TITLE   = "#f3f5f7"
BORDER  = "#1b2636"
...
def _from_row(key, default="—"):
    """Robust getter that works if player_row is a Series or a 1-row DataFrame."""
    try:
        if hasattr(player_row, "iloc"):
            # DataFrame-like
            return _to_str(player_row.iloc[0][key], default)
        # Series-like
        return _to_str(player_row[key], default)
    except Exception:
        return default

# Toggle
show_top = st.checkbox("Show top profile header", value=True)

if show_top:
    # Try to prefill from player_row; allow easy overrides
    colA, colB, colC = st.columns([1.2, 2.2, 2.0])
    with colA:
        position = st.text_input("Position", _from_row("Position", "CM/DM/RW"))
    with colB:
        name = st.text_input("Name", _from_row("Player", _from_row("Name", "David Kasumu")))
    with colC:
        team = st.text_input("Team", _from_row("Team", "Huddersfield"))

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        age = st.text_input("Age", _from_row("Age", "25"))
    with col2:
        games = st.text_input("Games", _from_row("Games", _from_row("Apps", "32")))
    with col3:
        goals = st.text_input("Goals", _from_row("Goals", "3"))
    with col4:
        assists = st.text_input("Assists", _from_row("Assists", "1"))

    # Render header block (22px + 10px) in your dark theme
    st.markdown(
        f"""
        <div style="
            background:{PAGE_BG};
